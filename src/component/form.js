...
const MainForm = () => {
  ...
  const diffYear = (date, today) => {
    if (date != null) {
      ...
      if (today.getMonth() >= minMonth && today.getDate() >= minDay) {
        if (age >= 65) {
          return <div style={{ color: "green" }}>สามารถเข้ารับบริการได้</div>;
        } else if (age === 64) {
          return (
            <div>
              <div style={{ color: "red" }}>
                ไม่สามารถเข้ารับบริการได้ เนื่องจากอายุจะครบ 65 ปี
              </div>
          ...
          return (
            <div>
              <div style={{ color: "red" }}>
                ไม่สามารถเข้ารับบริการได้ เนื่องจากไม่ตรงตามเงื่อนไข
              </div>
        ...
        return (
          <div>
            <div style={{ color: "red" }}>
              ไม่สามารถเข้ารับบริการได้ เนื่องจากอายุไม่ถึง 6 เดือน
            </div>
          </div>
        );
      } else if (age - 1 >= 65) {
        ...
        return (
          <div>
            ...
            <div>
              {" "}
              ในวันที่ {thaiDate} {currentYear + 543} ที่กำลังจะถึงนี้
  ...
  const handleName = (e) => {
    ...
    // Allow only letters (Thai and Latin) and spaces
    ...
    if (input === "" || validPattern.test(input)) {
      ...
      // keep the value but set an error so user knows it's invalid
      setName(input);
      setNameError("ห้ามใส่ตัวเลข หรืออักษรพิเศษ");
    }
  };
  ...
  const handleSubmis = () => {
    ...
    // Prevent submission if name contains invalid characters
    if (nameError) {
  ...
  return (
    <div className="container">
      <div className="card">
        <div className="card-header text-hearder">
          ลงทะเบียนรับวัคซีน
          <FontAwesomeIcon icon={faUserNurse} style={{ paddingLeft: "15px" }} />
          <div className="aboutText">
            กิจกรรมการฉีดวัคซีนป้องกันโควิด 19
            เปิดให้ผู้รับบริการสามารถเข้ารับบริการฉีดวัคซีน ในวันที่ 1 มิถุนายน
            พ.ศ.2566 – 31 สิงหาคม พ.ศ.2566 โดยมีเงื่อนไขการเข้ารับวัคซีน ดังนี้
            ผู้สูงอายุ 65 ปีขึ้นไป (ชาย,หญิง) เด็กที่มีอายุระหว่าง 6 เดือน ถึง 2
            ปี (ชาย,หญิง) โดยหากผู้มารับบริการมีอายุไม่ตรงตามเงื่อนไข
            หรือมารับบริการไม่ตรงช่วงเวลาที่มีสิทธิ์ จะไม่สามารถรับวัคซีนได้
          </div>
        </div>
        ...
        />
        {nameError && (
          <div style={{ color: "red", marginTop: "6px" }}>{nameError}</div>
        )}
        ...
        <div className="customtext">
          วันเดือนปีเกิด: {date && formatSelectedDate(date)}
        </div>
        ...
        <Modal show={showEmptyFieldsModal} onHide={handleCloseEmptyFieldsModal}>
          ...
          <Modal.Body>
            ...
            {(!name || name === "") && (
              <div style={{ color: "red" }}>ชื่อ-นามสกุล</div>
            )}
            {(!passid || passid === "") && (
              <div style={{ color: "red" }}>บัตรประชาชน</div>
            )}
            {(!gender || gender === "") && (
              <div style={{ color: "red" }}>เพศ</div>
            )}
        ...
        <Modal
          ...
          onHide={handleCloseSubmittedDataModal}
        >
          <Modal.Header closeButton>
          ...
          <Modal.Footer>
            <button
              ...
              onClick={handleCloseSubmittedDataModal}
            >
              ย้อนกลับ
            </button>
          </Modal.Footer>
